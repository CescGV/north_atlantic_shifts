...
pdf("figures/lat_dif_final.pdf", width = 4, height = 8)
...
cor.test(slopes_bar$thermal_lat_s, slopes_bar$real_lat_s)   
cor.test(slopes_now$thermal_lat_s, slopes_now$real_lat_s)
cor.test(slopes_nor$thermal_lat_s, slopes_nor$real_lat_s) 
...
#  Higly corrected and still significant
...
summary(gls(real_lat_s ~ Troph + records, weights = varPower(form = ~ records + Troph), data = bar_trait))
summary(gls(real_lat_s ~ DemersPelag + records, weights = varPower(form = ~ records), data = bar_trait)) #
...
summary(gls(real_lat_s ~ Zoo_final + records, weights = varPower(form = ~ records), data = bar_trait)) # 
...
summary(lm(real_lat_s ~ TempPrefMean, data = nor_trait)) # significant
summary(lm(real_lat_s ~ DepthMax, data = nor_trait)) # Significant
sjPlot::plot_model(lm(real_lat_s ~ DepthMax, data = nor_trait), "pred", show.data = T)
...
summary(lm(real_lat_s ~ Fecundity, data = nor_trait)) # Significant for one big value
summary(lm(real_lat_s ~ Fecundity, data = filter(nor_trait, Fecundity < 40000000))) # Significant for one big value
summary(lm(real_lat_s ~ AgeMatMin, data = nor_trait))
summary(lm(real_lat_s ~ DemersPelag, data = nor_trait))
summary(lm(real_lat_s ~ slimit_book, data = nor_trait))
...
summary(lm(real_lat_s ~ DemersPelag, data = now_trait)) # Almost significant
...
summary(lm(real_lat_s ~ Fisheries, data = now_trait)) # Almost significant
...
model5 = gls(real_lat_s ~ TempPrefMean, data = nor_trait)
model6 = gls(real_lat_s ~ DepthMax, data = nor_trait)

summary(model5)
summary(model6)

sjPlot::plot_model(model5, "pred", terms = "slimit_book", show.data = T)
a = predictorEffects(model0)$TempPrefMean
b = predictorEffects(model1)$Zoo_final
c = predictorEffects(model2)$DemersPelag
d = predictorEffects(model3)$TempPrefMean
e = predictorEffects(model4)$DemersPelag
f = predictorEffects(model5)$TempPrefMean
g = predictorEffects(model6)$DepthMax

pdf("figures/Figures_traits_2.pdf", width = 10, height = 9)
gridExtra::grid.arrange(plot(a), plot(b),
                        plot(c), plot(d), plot(e),plot(c),
                        plot(f), plot(g), nrow = 3)
dev.off()
