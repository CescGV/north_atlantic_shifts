...
raw_data = read.csv("../data/data_ready.csv") %>% filter(Year < 2021 & Year > 1993)
raw_data = raw_data %>% rename(real_depth = Bot_depth)
dat_yr_env = NULL
...
ggplot(dat_yr_env[is.na(dat_yr_env$SBT),]) + geom_point(aes(x = Longitude, y = Latitude, col = "SBT"))
...
# All sites
PA_sites = PA_data %>% dplyr::select(Longitude, Latitude) %>% distinct() %>% 
  mutate(Site = paste(Longitude, Latitude))


# Species
PA_sp = PA_data %>% dplyr::select(Species) %>% distinct()

#Species per site
PA_species = PA_data %>% dplyr::select(Species, Longitude, Latitude, Abundance) %>%
  distinct() %>% mutate(Site = paste(Longitude, Latitude), Abundance = 1) %>% dplyr::select(-Longitude, -Latitude)

#Env data
env_data = PA_data %>% dplyr::select(Longitude, Latitude, Bot_depth, SST, SBT, ICC) %>% distinct()
...
bar_chan = bar_chan %>% dplyr::filter(Species %in% selected_sp$Species[selected_sp$Region == "Barents Sea"])
now_chan = now_chan %>% dplyr::filter(Species %in% selected_sp$Species[selected_sp$Region == "Norwegian Sea"])
nor_chan = nor_chan %>% dplyr::filter(Species %in% selected_sp$Species[selected_sp$Region == "North Sea"])
...
boxplot(slopes$lat_s ~ slopes$Region)
abline(h = 0)
...
a = slopes %>% 
  mutate(agreement = case_when(thermal_lat_s > 0 ~ 1,
                               thermal_lat_s < 0 ~ 0),
         agreement_real = case_when(real_lat_s > 0 ~ 1,
                                    real_lat_s < 0 ~ 0))

table(a$Region, a$agreement)
table(a$Region, a$agreement_real)

(33/35)*100; (22/26)*100; (25/58)*100
...
realized_slope = realized_slope %>% dplyr::select(Species, n, Region, lat_s, lat_p, lat_dev) %>% 
